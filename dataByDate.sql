-- Пример: Загрузка за дату '2024-01-01'. Можно заменить на переменную или параметр.
TRUNCATE TABLE memory.tpch_tiny_dds.temp_target_date ;
INSERT INTO memory.tpch_tiny_dds.temp_target_date  (date_value) SELECT date '1994-12-07' AS date_value;
...
JOIN memory.tpch_tiny_dds.temp_target_date t ON o.orderdate = t.date_value;
...
JOIN tpch.tiny.lineitem l ON p.partkey = l.partkey
...
SELECT DISTINCT
    lower(to_hex(sha256(cast(cast(c.custkey AS VARCHAR) AS VARBINARY)))) AS customer_nation_hk,
    lower(to_hex(sha256(cast(cast(c.custkey AS VARCHAR) AS VARBINARY)))) AS customer_hk,
    lower(to_hex(sha256(cast(cast(n.nationkey AS VARCHAR) AS VARBINARY)))) AS nation_hk,
    now() AS load_date,
    'tpch.tiny' AS source
...
JOIN tpch.tiny.part p ON ps.partkey = p.partkey
JOIN tpch.tiny.lineitem l ON p.partkey = l.partkey
JOIN tpch.tiny.orders o ON l.orderkey = o.orderkey
JOIN memory.tpch_tiny_dds.temp_target_date t ON o.orderdate = t.date_value;
